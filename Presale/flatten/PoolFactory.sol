...
contract Pool is OwnableUpgradeable {
    ...
    function upgradeHardCap(uint256 _hardCap) external onlyOwner {
        hardCap = _hardCap;
    }
...
contract PoolFactory is OwnableUpgradeable {
    ...
    enum Tier { Bronze, Silver, Gold, Platinum, Diamond  } // Enum
    ...
    constructor(uint256 fee1, uint256 fee2, address _feeWallet, address lockContract) {
        fees[0] = fee1; // 2
        fees[1] = fee2; // 2
        tvl = 0;
        feeWallet = payable(_feeWallet);
        lock = IKronosLock(lockContract);
    }

    function setHardCapTier(Tier tier, uint256 amount) private {
        require(amount > 0);
        tierWiseHardCap[tier] = amount;
    }

    function setSilver(uint256 _amount) external onlyOwner {
        setHardCapTier(Tier.Silver, _amount);
    } 

    function setBronze(uint256 _amount) external onlyOwner {
        setHardCapTier(Tier.Bronze, _amount);
    } 
    
    function setGold(uint256 _amount) external onlyOwner {
        setHardCapTier(Tier.Gold, _amount);
    } 

    function setPlatinum(uint256 _amount) external onlyOwner {
        setHardCapTier(Tier.Platinum, _amount);
    } 

    function setDiamond(uint256 _amount) external onlyOwner {
        setHardCapTier(Tier.Diamond, _amount);
    } 
    ...
    function createPool(
        ...
        uint256[2] memory _rateSettings, // [0] = rate, [1] = uniswap rate
        uint256[2] memory _contributionSettings, // [0] = min, [1] = max
        uint256[2] memory _capSettings,// [0] = soft cap, [1] = notihng
        uint256[3] memory _timeSettings, // [0] = start, [1] = end, [2] = unlock seconds
        ...
        _capSettings = [ _capSettings[0], tierWiseHardCap[Tier.Bronze]];
